...
context_ptr on_tls_init(tls_mode mode, std::string basename, websocketpp::connection_hdl hdl) {
	...
	std::cout << "Pem file : " << basename << ".crt" << std::endl;
	std::cout << "create asio context shared ptr" << std::endl;
	
    context_ptr ctx = websocketpp::lib::make_shared<asio::ssl::context>(asio::ssl::context::sslv23);

    try {
        if (mode == MOZILLA_MODERN) {
			std::cout << "Set options MODERN" << std::endl;
			...
			std::cout << "Set options INTERMEDIATE" << std::endl;
		...
		std::cout << "Set callback fort password" << std::endl;

        ctx->set_password_callback(bind(&get_password));

		std::cout << "Set perm crt : " << basename << std::endl;
        ctx->use_certificate_chain_file(basename + ".crt");

		std::cout << "Set perm key" << std::endl;
...
//        ctx->use_tmp_dh_file("dh.pem");
		...
		std::cout << "Set SSL_CTX_set_cipher" << std::endl;
