...
context_ptr on_tls_init(tls_mode mode, websocketpp::connection_hdl hdl, std::string basename) {
...
WebsocketServer* ServerInit(std::function<void(WebsocketServer*, websocketpp::connection_hdl)> con_callback,
  ...
  websocket_server->set_tls_init_handler(bind(&on_tls_init,MOZILLA_INTERMEDIATE,::_1, basename));
